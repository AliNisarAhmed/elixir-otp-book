defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    test "templates are unique until cycle repeats", %{quiz: quiz} do
      ...
      assert current_template(first_quiz) != current_template(second_quiz)

      assert current_template(reset_quiz) in [
               current_template(first_quiz),
               current_template(second_quiz)
             ]
    end
  end

  describe "a quiz that always adds one & two" do
